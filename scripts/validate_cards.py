...
# Check 2: No stray card-title/tags divs sitting between card closing tag and next card opener.
# The only real failure mode: </a>\n...<div class="card-title"> or </div></div>\n<div class="card-title">
# without a card opener in between. Use a direct pattern for this.
...
# Check 3: Every card-title is preceded by either <a class="card or <div class="card-top">
# (card-top always comes before card-title inside a card)
for m in re.finditer(r'<div class="card-title">', c):
    pos = m.start()
    before = c[max(0, pos-300):pos]
    if '<div class="card-top">' not in before:
        errors.append(f"card-title with no card-top in preceding 300 chars at {pos}: {repr(c[pos:pos+50])}")
